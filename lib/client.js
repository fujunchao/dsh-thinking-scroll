...
`;
    ...
    function ThinkingScrollSettingsCard({ view, scope, t }) {
      ...
      const save = async () => {
        ...
        setFailed(false);
        try {
          if (draft.maxHeightVh !== baseline.maxHeightVh) {
            if (!(await scope.set("maxHeightVh", draft.maxHeightVh))) throw new Error("settings write was rejected");
          }
          if (draft.autoFollow !== baseline.autoFollow) {
            if (!(await scope.set("autoFollow", draft.autoFollow))) throw new Error("settings write was rejected");
          }
    ...
    function configValue(entry) {
      ...
      if (typeof entry.get === "function") return entry.get();
      return entry;
    }
    ...
    function apply(ctx, config) {
      ...
      currentConfig = normalizeConfig({
        ...DEFAULTS,
        maxHeightVh: configValue(source.maxHeightVh),
        autoFollow: configValue(source.autoFollow),
      });
      installStyles();
      applyConfigToDom();
      ...
      ctx.effect(() => ctx.configForms.whileServed([SETTINGS_NAMESPACE], () =>
        ctx.slots.inject("plugins.item", () => ctx.slots.register({
          ...
          inject: () => ({ scope, t }),
        }, ThinkingScrollSettingsCard))), "thinking-scroll: settings card");
    }
